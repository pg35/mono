...
export function listReducer<T extends { id: Id }>(
  state: List<T>,
  action: ListAction<T>,
): List<T> {
  switch (action.type) {
    ...
    case "remove_item":
      const newState = {
        ...state,
        items: { ...state.items },
        itemOrder: state.itemOrder.filter((itemId) => itemId !== action.itemId),
        selected: state.selected === action.itemId ? null : state.selected,
      };
      delete newState.items[action.itemId];
      return newState;
    ...
    case "swap_items":
      const newState2 = { ...state, itemOrder: [...state.itemOrder] };
      const temp = newState2.itemOrder[action.bItemId];
      newState2.itemOrder[action.bItemId] = newState2.itemOrder[action.aItemId];
      newState2.itemOrder[action.aItemId] = temp;
      return newState2;
    default:
      const _exhaustiveCheck: never = action;
